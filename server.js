...
const activeCalls = new Map(); // Track active calls by appointment ID (in-memory)
app.get('/', (req, res) => {
    res.send('Healthcare Plus WebRTC Signaling Server is running.');
});
...
io.on('connection', (socket) => {
    ...
    socket.on('start-call', ({ appointmentId, meetingId, doctorName, duration }) => {
        ...
        // NEW: Store call data in Firebase using consolidated structure
        storeCallData(appointmentId, {
            meetingId,
            startTime: now.toISOString(),
            endTime: endTime.toISOString(),
            duration: duration || 30
        }).catch(err => {
            console.error('Failed to store call data in Firebase:', err);
        });
    ...
    socket.on('end-call', ({ appointmentId }) => {
        ...
        // NEW: Update call status in Firebase
        updateCallStatus(appointmentId, 'ended').catch(err => {
            console.error('Failed to update call status in Firebase:', err);
        });
    ...
    socket.on('join-meeting', ({ meetingId, participantId, participantName, isHost, appointmentId }) => {
        ...
        if (!meetings.has(meetingId)) {
            ...
            // Store meeting data in Firebase with appointment ID (uses NEW structure)
        ...
        if (isHost && currentParticipants.length > 0) {
            console.log(`🏥 Doctor ${participantName} rejoined meeting ${meetingId}`);
            socket.to(meetingId).emit('doctor-rejoined', {
...
function handleParticipantLeave(socket, meetingId, participantId) {
    ...
    if (participant) {
        ...
        if (isHost) {
            console.log(`🏥 Doctor ${participantNameLeaving} left meeting ${meetingId}`);
            socket.to(meetingId).emit('doctor-left', {
                doctorName: participantNameLeaving,
                meetingId: meetingId,
                appointmentId: appointmentId
            });
            // NEW: Update call status when doctor leaves
            if (appointmentId) {
                updateCallStatus(appointmentId, 'ended').catch(err => {
